---
title: "02 Data wrangling"
---

# 1) Setup  
## Loading Packages
```{r Setup, message=F}
# Loading packages (every time you open R)
library(readxl) # to read excel files
library(janitor) # to clean data
library(dplyr) # wrangling
library(tidyr) # wrangling
library(readr) # to export csv

```

## Reading Data

```{r Reading data}
soyk <- read_excel("../data/01_soybeans.xlsx")

# Checking first rows
soyk

# View
View(soyk)

```

This study was conducted at four different locations in one year to assess the effect of potassium (K) fertilizer rates on soybean trifoliate K concentration at growth stage R4.  

Each study was a randomized complete block design (RCBD) with K rate as the only treatment factor with levels 0, 50, 100, and 150 lbs K2O/ac.

```{r Glimpse}
glimpse(soyk)
```

```{r Summary}
summary(soyk)
```

# 2) Data wrangling  
```{r soyk_1 - clean_names()}
soyk_1 <- clean_names(soyk)

soyk_1
```


```{r soyk_2 - filter()}
names(soyk_1)

soyk_2 <- filter(soyk_1, 
                 k_rate_lbac == 0
                 )
# Boolean operators for r:
## == is "equal to"
## != is "not equal to"
soyk_2
```

```{r soyk_3 - mutate()}
# Transforming K rate from lbs/ac to kg/ha
soyk_3  <- mutate(soyk_1,
                  k_rate_kgha = k_rate_lbac*0.454/0.405,
                  k_rate_kgha = round(k_rate_kgha, 0)
                  )

soyk_3
View(soyk_3)
```

```{r soyk_4 - case_when()}
soyk_4 <- mutate(soyk_3,
                 rep = case_when(
                   plot > 100 & plot < 200 ~ 1,
                   plot > 200 & plot < 300 ~ 2,
                   plot > 300 & plot < 400 ~ 3,
                   plot > 400 & plot < 500 ~ 4
                 )
                 )

soyk_4

```

```{r soyk_5 - pivot_longer()}
names(soyk_4)

soyk_5 <- pivot_longer(soyk_4,
                       names_to = "location_k",
                       values_to = "k_pct",
                       cols = contains("k_pct")
                       )

soyk_5

View(soyk_5)
```

```{r soyk_6 - separate()}
soyk_6 <- separate(soyk_5,
                   location_k,
                   into = c("location", "k", "pct")
                   
                   )
soyk_6

#View(soyk_6)
```

```{r soyk_7 - select()}
names(soyk_6)

soyk_7 <- select(soyk_6,
                 location, k_rate_kgha, rep, k_pct
                 )

soyk_7
```

```{r soyk_8 - arrange()}
soyk_8 <- arrange(soyk_7,
                  location
                  )

## arrange goes by increasing number or alphabetical; to flip the order, put inside function desc(xyz)

soyk_8
```

```{r soyk_9 and 10 - group_by and summarise}
# group_by
soyk_9 <- group_by(soyk_8,
                   location,
                   k_rate_kgha
                   )

soyk_9

class(soyk_8)

class(soyk_9)

# summarise
soyk_10 <- summarize(soyk_9,
                     meank_pct = mean(k_pct)
                     ## could also use other summary functions like median, minimum, max, etc.
                     )

soyk_10
```
pipe function:
feeds object a as argument into function b
a |> b()
original was a %>% b()

```{r Piping it all together}
# w = "wrangled" 
soyk_w <- soyk %>% 
  clean_names() %>% 
  mutate(k_rate_kgha = k_rate_lbac*0.454/0.405,
                  k_rate_kgha = round(k_rate_kgha, 0)) %>% 
  mutate(rep = case_when(
                   plot > 100 & plot < 200 ~ 1,
                   plot > 200 & plot < 300 ~ 2,
                   plot > 300 & plot < 400 ~ 3,
                   plot > 400 & plot < 500 ~ 4)
                 ) %>%
  pivot_longer(names_to = "location_k",
                       values_to = "k_pct",
                       cols = contains("k_pct")) %>%
  separate(location_k,
                   into = c("location", "k", "pct")) %>%
  select(location, k_rate_kgha, rep, k_pct) %>% 
  arrange(location)

soyk_w


```

```{r Checking if same as without pipe}
# Same result?
identical(soyk_8, soyk_w)

```

```{r Saving to file for further analysis}
write_csv(soyk_w,
          "../output/soybean_processed.csv")

```

# 3) Assignment   
Starting from soyk, use data wrangling functions in a pipe workflow to generate the average k_pct for each K rate (in kg/ha) within each location (basically, averaging over reps). You can use the chunk below to complete this quiz.

```{r assignment}
soyk_f <- soyk %>% 
  clean_names() %>% 
  mutate(k_rate_kgha = k_rate_lbac*0.454/0.405,
                  k_rate_kgha = round(k_rate_kgha, 0)) %>% 
  mutate(rep = case_when(
                   plot > 100 & plot < 200 ~ 1,
                   plot > 200 & plot < 300 ~ 2,
                   plot > 300 & plot < 400 ~ 3,
                   plot > 400 & plot < 500 ~ 4)
                 ) %>%
  pivot_longer(names_to = "location_k",
                       values_to = "k_pct",
                       cols = contains("k_pct")) %>%
  separate(location_k,
                   into = c("location", "k", "pct")) %>%
  select(location, k_rate_kgha, rep, k_pct) %>% 
  arrange(location) %>%
  group_by(location,
                   k_rate_kgha) %>%
  summarize(meank_pct = mean(k_pct))

soyk_f

```


Tip: the final data frame will have 16 rows.

Submit your result (take a screenshot that includes the code and the chunk output) on eLC under assignment #2 by Friday Jan 17th 11:59 pm.

install.packages("usethis")

library(usethis)

use_git_config(user.name = "Mica-Jones", user.email = "micahdjones2001@gmail.com")

gitcreds::gitcreds_set()

